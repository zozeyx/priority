...
void add_to_schedule(struct Task* task, int first) {
    if(first == 1) {
        ...
        if(tail_task != NULL && task->priority > tail_task->priority) {
        ...
        if(task->next != NULL && global_time-process_starttime[task->process_number] >= ROBINTIME) {
            head_task = task->next;
            tail_task->next = task;
            task->next = NULL;
            task->priority = tail_task->priority+ROBINTIME;
            tail_task = task;
        }
        if(task != head_task) {
            sprintf(buf, "P%d (%d-%d)\n", task->process_number+1, process_starttime[task->process_number], global_time);
            strcat(gant, buf);
            process_starttime[task->process_number] = -1;
        }
    }
}
...
void* process_task(void* args) {
    ...
    while(process_runtime[input_task->process_number] < execution_time[input_task->process_number]) {
        ...
        if(input_task == head_task) {
            ...
            global_time++;
            if(process_runtime[input_task->process_number] < execution_time[input_task->process_number]) add_to_schedule(input_task, 0);
            cnt++;
            is_timing = 1;
        }
    }
...
void create_new_process(int num) {
    ...
    else newp->priority = tail_task->priority+ROBINTIME;
    newp->next = NULL;
